...
#' Module UI for the sensitivity analysis page.
#' 
#' @param id ID of the module
#' @return Div for the sensitivity analysis page
SensitivityAnalysisPageUi <- function(id) {
  ns <- NS(id)
  ...
  div(
    ...
    sidebarLayout(
      sidebarPanel = sidebarPanel(
        uiOutput(ns("SA_input")),
        actionButton(inputId = ns("SA_reset"), label = "Reset all inputs")
      ),
      mainPanel = mainPanel(
        tabsetPanel(
          tabPanel(
            ...
            downloadButton(outputId = ns("downloadTable2"), label = "Download Table"),
            br(),
            br(),
...
#' Module server for the sensitivity analysis page.
#' 
#' @param id ID of the module
#' @param data Reactive containing data frame.
SensitivityAnalysisPageServer <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$SA_input <- renderUI({
      ...
      div(
        ...
        checkboxInput(inputId = ns("prevcheck2"), label = "Display disease prevalence"),
        checkboxInput(inputId = ns("weightcheck2"), label = "Display percentage weights"),
        checkboxGroupInput(
          inputId = ns("mod_removal"),
          label = "Select which model to display",
          choices = c(
            "Original model" = 1,
        ...
        uiOutput(outputId = ns("ci_colour2")),
        uiOutput(outputId = ns("QAoption2")),
        uiOutput(outputId = ns("covariate2")),
        uiOutput(outputId = ns("covariate_option2")),
        checkboxGroupInput(
          inputId = ns("statscheck2"),
    ...
    output$covariate2 <- renderUI({
      ...
      if ((C > 6 & Names[7] != "rob_PS") | (C > 13 & Names[13] == "ac_RS")) {
        ...
        selectInput(inputId = ns("covcheck2"), label = "Display covariates", choices = choicesCov, selected = 1)
      }
    })  
    ...
    output$trials <- renderUI({
      ...
      #list of study names
      checkboxGroupInput(
        inputId = ns("triallist"),
        label = h4("Select studies to include:"),
        choices = choicesTrials,
        selected = choicesTrials
        )
    })
    ...
    output$downloadTable2 <- downloadHandler (
      ...
      content = function(file){
        write.table(table2(), file, sep=",", row.names=FALSE)
      }
    )
    
    # Plot an interactive SROC curve for sensitivity analysis
    DrawRocPlotSa <- function() {
      if(is.null(data())){return()}
      else
        X <- data()
    ...
    output$sensplot <- renderUI ({
      plotOutput(outputId = ns("sroc_sa"), height = "750px", width = "750px", click = clickOpts(ns("plot_click_ma2")))
    })
    ...
    output$downloadSATable <- downloadHandler(
      ...
      content = function(file){
        write.table(sa_statTable(), file, sep=",", row.names=FALSE)
      }
    )
    
    # Stats for decision modelling distribution
    DecisionModel2 <- reactive({
      if(is.null(data())){return()}
      else
        X <- data()
      X <- X[input$triallist, ]
    ...
    output$downloadParameters2 <- downloadHandler(
      ...
      content = function(file){
        write.table(DecisionModel2(), file, sep=",", row.names=FALSE)
      }
    )
    ...
    output$downloadRevMan2 <- downloadHandler(
      ...
      content = function(file){
        write.table(revman2(), file, sep=",", row.names=FALSE)
      }
    )
