...
AnalysisPageServer <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    statTable <- reactive({
      ...
      Z = data.frame(estimate = c(logit_Sens[1], logit_Spec[1], Sens[1], Spec[1], fpr[1], fnr[1],
                                  DOR , LRp , LRn, correlation, Theta, Lambda, beta, sigma2_theta, sigma2_alpha),
                     lci = c(logit_Sens[2], logit_Spec[2], Sens[2], Spec[2], fpr[2], fnr[2],
                             exp(log(DOR)-qnorm(0.975)*logse.DOR) , exp(log(LRp)-qnorm(0.975)*logse.LRp), 
                             exp(log(LRn)-qnorm(0.975)*logse.LRn), 9999, 9999, 9999, 9999, 9999, 9999),
                     uci = c(logit_Sens[3], logit_Spec[3], Sens[3], Spec[3], fpr[3], fnr[3],
                             exp(log(DOR)+qnorm(0.975)*logse.DOR), exp(log(LRp)+qnorm(0.975)*logse.LRp), 
                             exp(log(LRn)+qnorm(0.975)*logse.LRn), 9999, 9999, 9999, 9999, 9999, 9999),
                     row.names=c("Logit Sens", "Logit Spec", "Sensitivity", "Specifcity", "FPR", "FNR",
                                 "DOR", "LR+" , "LR-", "Correlation", "Theta", "Lambda", "beta", 
      ...
      if ('5' %in% input$statscheck) {statticks[5] <- T} 
      if ('6' %in% input$statscheck) {statticks[6:10] <- T} 
      if ('7' %in% input$statscheck) {statticks[11] <- T} 
      if ('8' %in% input$statscheck) {statticks[12:13] <- T} 
      if ('1' %in% input$statscheck) {statticks[14] <- T} 
      if ('2' %in% input$statscheck) {statticks[15] <- T} 
